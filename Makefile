...
ROCM_SRCS := $(wildcard src/rocm/*.cuh)
...
ROCM_ARCH ?= gfx1151
ROCM_CFLAGS ?= -O3 -ffast-math -g -fno-finite-math-only -pthread -D__HIP_PLATFORM_AMD__ -DDFLASH_WAVE_SIZE=32 -Wno-unused-command-line-argument --offload-arch=$(ROCM_ARCH)
...
gguf-tools/quality-testing/score_official: gguf-tools/quality-testing/score_official.c ds4.h $(CORE_OBJS) rax.o ds4_gpu_args.o
	$(CC) $(QUALITY_CFLAGS) -I src -o $@ gguf-tools/quality-testing/score_official.c $(CORE_OBJS) rax.o ds4_gpu_args.o $(METAL_LDLIBS)
...
tests/rocm_smoke.o: tests/rocm_smoke.c ds4_gpu.h
	$(HIPCC) $(ROCM_CFLAGS) -I src -c -o $@ $<

tests/rocm_smoke: tests/rocm_smoke.o ds4_rocm.o ds4_rocm_compat.o ds4_rocm_unavailable.o
...
gguf-tools/quality-testing/score_official.o: gguf-tools/quality-testing/score_official.c ds4.h
	$(CC) $(filter-out -ffast-math,$(QUALITY_CFLAGS)) -I src -c -o $@ $<
...
ds4_test.o: tests/ds4_test.c src/ds4_server.c src/ds4.h src/ds4_ssd.h src/ds4_distributed.h src/ds4_help.h src/ds4_kvstore.h src/rax.h
	$(CC) $(CFLAGS) -I src -Wno-unused-function -c -o $@ $<

ds4_agent_test.o: tests/ds4_agent_test.c src/ds4_agent.c src/ds4.h src/ds4_ssd.h src/ds4_distributed.h src/ds4_help.h src/ds4_kvstore.h src/ds4_web.h src/linenoise.h
	$(CC) $(CFLAGS) -I src -Wno-unused-function -c -o $@ $<
...
tests/test_gpu_args.o: tests/test_gpu_args.c ds4_gpu_args.h ds4_gpu_mgpu.h
	$(CC) $(CFLAGS) -I src -DDS4_NO_GPU -c -o $@ $<
...
tests/test_gpu_xdev.o: tests/test_gpu_xdev.c ds4_gpu.h ds4_gpu_mgpu.h
	$(CC) $(CFLAGS) -I src -I$(CUDA_HOME)/include -c -o $@ $<
...
ds4_cuda_test_hooks.o: ds4.c ds4.h ds4_gpu.h ds4_gpu_mgpu.h ds4_layer_pack.h
	$(CC) $(CFLAGS) -Wno-unused-function -DDS4_TEST_HOOKS -I$(CUDA_HOME)/include -c -o $@ $<
...
tests/test_engine_mgpu_runtime.o: tests/test_engine_mgpu_runtime.c ds4.h ds4_gpu_mgpu.h
	$(CC) $(CFLAGS) -DDS4_TEST_HOOKS -I src -I$(CUDA_HOME)/include -c -o $@ $<
...
tests/test_sampling.o: tests/test_sampling.c ds4.h
	$(CC) $(CFLAGS) -DDS4_TEST_HOOKS -I src -c -o $@ $<
